...
class SearchShopModal extends ConsumerStatefulWidget {
  const SearchShopModal({Key? key}) : super(key: key);

  @override
  ConsumerState<SearchShopModal> createState() => _SearchShopModalState();
}

class _SearchShopModalState extends ConsumerState<SearchShopModal> {
  @override
  void initState() {
    super.initState();
    Future.delayed(
      Duration.zero,
      () {
        ref.read(addOrderProvider.notifier).fetchShops(
              query: '',
              checkYourNetwork: () {
    ...
    return Material(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            SearchTextField(
              onChanged: (value) => notifier.setShopQuery(context, value.trim()),
